...
namespace nocte { 
	...
	public:
		...
		Param( const std::string &name, void *param, ParamType paramType, bool show = false ) :
			mName(name), mParam(param), mType(paramType), mShow(show)
		{
		};
	...
	public:
		...
		void drawDebug()
		{
			...
			textLayout.setFont( ci::Font("Arial", 11) );
	...
	private:
		...
		void	addOrBind( const std::string &name, void *param, Param::ParamType paramType )
		{
			Param *p = getParam(name);
			if ( p )
				p->mParam = param;
			else
				mParams.push_back( new Param( name, param, paramType ) );
		}
